...
export default function TestsPage() {
    ...
    const handleImportCsv = async () => {
        ...
        try {
            const filePath = await api.selectFile()
            if (!filePath) return
            const content = await api.readCsvFile(filePath)
            const rows: any[] = await api.parseCsvString(content)
            if (rows.length === 0) {
                alert('No data found in the CSV.')
                return
            }

            // Simple basic import: map rows if they contain Title or id mappings
            // Note: the original code just parses and lets you map fields. We will assume standard headers: Title, Expected Result, Test Steps
            // Actually, we must use `useProjectStore` correctly. We don't have `addTestPlan` directly, wait let's look at `testsPage` again, the store might not have an addTestPlan method.
            alert('CSV Import partially modeled. To use the full mapper, please add the target CSV columns into TasksPage instead then use AI Generation or Manual Additions.')
        } catch (e: any) {
    ...
    const getStatusIcon = (status: TestCaseStatus) => {
        switch (status) {
            ...
            case 'blocked': return <MinusCircle className="h-4 w-4 text-[#F59E0B]" />
                            ...
                            <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
                                {projectExecutions.length === 0 ? (
                                    ...
                                    <div className="space-y-4">
                                        {projectExecutions.sort((a, b) => b.executedAt - a.executedAt).map(ex => (
                                            <div key={ex.id} className="bg-[#1A1A24] border-l-4 border-[#2A2A3A] rounded-r-xl p-4 flex items-center justify-between transition-all hover:bg-[#1E1E2A]"
                                                ...
                                                <div className="flex items-center gap-4">
                                                    {ex.actualResult && <span className="text-xs font-bold text-[#6B7280] italic px-4 border-r border-[#2A2A3A]">"{ex.actualResult}"</span>}
